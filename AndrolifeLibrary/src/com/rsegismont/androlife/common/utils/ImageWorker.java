...
public abstract class ImageWorker {
	...
	private ImageCacher mImageCache;
	private ImageCacher.ImageCacheParams mImageCacheParams;
	...
	protected void addToBadUrls(String urlString, int reqWidth, int reqHeight) {
		badUrls.add(urlString + reqWidth + reqHeight);
	}

	public void loadImage(Object data, ImageView imageView) {
		loadImage(data, imageView, DEFAULT_WIDTH, DEFAULT_HEIGHT);
	}
	 ...
	 */
	public void loadImage(Object data, ImageView imageView, int reqWdith, int reqHeight) {


		final boolean workCancelled = cancelPotentialWork(data, imageView);
		
		if (badUrls.contains("" + data + reqWdith + reqHeight) || (data == null)) {
		...
		if (mImageCache != null) {
			value = mImageCache.getBitmapFromMemCache(String.valueOf(data), reqWdith, reqHeight);
		}
		...
		if (value != null) {
			...
			task.executeOnExecutor(AsyncTask.DUAL_THREAD_EXECUTOR, data, reqWdith, reqHeight);
		}
	}
	 ...
	 */
	public void addImageCache(ImageCacher.ImageCacheParams cacheParams) {
		mImageCacheParams = cacheParams;
		mImageCache = new ImageCacher(mImageCacheParams);
	 ...
	 */
	protected abstract Bitmap processBitmap(Object data, int reqWidth, int reqHeight,Point originalValues);

	/**
	 * @return The {@link ImageCache} object currently being used by this ImageWorker.
	 */
	public ImageCacher getImageCache() {
		return mImageCache;
	}
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, BitmapDrawable> {
		...
		protected BitmapDrawable doInBackground(Object... params) {
			...
			if (mImageCache != null && !isCancelled() && getAttachedImageView() != null && !mExitTasksEarly) {
				bitmap = mImageCache.getBitmapFromDiskCache(dataString, Integer.valueOf("" + params[1]),
						Integer.valueOf("" + params[2]),point);
			}
			...
			if (bitmap == null && !isCancelled() && getAttachedImageView() != null && !mExitTasksEarly) {
				...
				bitmap = processBitmap(params[0], Integer.valueOf("" + params[1]), Integer.valueOf("" + params[2]),point);
			}
			...
			if (bitmap != null) {
				...
				if (mImageCache != null) {
					mImageCache.addBitmapToCache(dataString, drawable, Integer.valueOf("" + params[1]),
							Integer.valueOf("" + params[2]),point);
				}
			}
